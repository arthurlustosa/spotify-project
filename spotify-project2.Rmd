...
Analisando o gosto musical dos meus amigos, percebei que muitos deles estavam escutando umas músicas novas em suas playlists. E quando essas músicas começavam a tocar, não tinha ninguém que ficasse calado ou mesmo parado. Isso mesmo, estou falando delas, das cantoras sertanejas! Quem não sabe, nem que seja, um trechinho de alguma música de Marília Mendoça ou aquela do famoso cinquenta reais (Naiara Azevedo), pois é, não tem como não conhecer. Vendo como as cantoras sertanejas conquistaram seu espaço nos últimos tempo, decidi analisar se houve realmente um BUUM no sertanejo feminino, e desde quando começou. 
...
rainhas_do_sertanejo_playlist <- read.csv("rainhas_do_sertanejo_playlist.csv")
```
...
#ordenando dataset
ranking_cantoras_sertanejas$artist_ <- factor(ranking_cantoras_sertanejas$artist_, levels = unique(ranking_cantoras_sertanejas$artist_)[order(ranking_cantoras_sertanejas$popularity_, decreasing = TRUE)])
...
```{r, echo=F}
info_videos_geral <- read.csv("projetos/spotify-project/dados/info_videos_geral.csv")
rainhas_sertanejo <- read.csv("projetos/spotify-project/dados/rainhas_sertanejo.csv")
...
Com os dados prontos, finalmente vamos para as análise. Para melhor estudarmos os resultados, vamos organizar o dataset em períodos, teremos a média trimestral e anual do número de views de cada cantora.
...
resume_trimestral <- resume_rainhas_sertanejo %>%
  group_by(quarterYear) %>%
  summarise(mean_views = mean(as.numeric(views)))

resume_anual <- resume_rainhas_sertanejo %>%
    group_by(Nome, year) %>%
    summarise(mean_views = sum(as.numeric(views)))
```
...
<h4>**Média de Views por Trimestre**</h4>

Visando normalizar os dados entre os anos, já que estamos ainda chegando na metade de 2017, a visualização abaixo é referente a média de views por trimestre.
```{r, echo=F, message=F, warning=F,fig.width=9, fig.height=5}
plot_ly(resume_trimestral, x = ~quarterYear, y = ~mean_views, type = 'scatter', mode = 'lines+makers',
        line = list(color = 'rgb(205, 12, 24)', width = 4)) %>%
  layout(title = "Média de Views por Trimestre", 
         xaxis = list(title="Ano", autotick=F),
         yaxis = list (title = "Total de Views"))

```

Analisando separadamente cada trimestre, vemos que o aumento do consumo de música continua crescendo. E a partir do terceiro trimestre de 2015 esse aumento é consideravelmente alto. 
...
Além da conclusão sobre o tema, esse trabalho tem sua importância pois trabalhamos com duas APIs bastante famosas, aprendemos a acessar e analisar os dados do Spotify, do YouTube, trabalhamos com várias bibliotecas de manipulação de dados e de datas e as bibliotecas gráficas plotly e streamgraph. 
