...
resume_rainhas_sertanejo <- rainhas_sertanejo %>%
  mutate(ano = as.Date(format(rainhas_sertanejo$publishedAt,  format = "%y"))) %>%
  group_by(Nome, ano) %>%
...
resume_trimestral <- rainhas_sertanejo %>%
  mutate(quarterYear =  as.yearqtr(as.Date(rainhas_sertanejo$publishedAt)),
         ano = as.Date(format(rainhas_sertanejo$publishedAt, format= "%Y")))

resume1 <- resume_trimestral %>%
  group_by(Nome, ano, quarterYear) %>%
  summarise(n = sum(views))

resume2 <- resume_trimestral %>%
  mutate(Nome = "Cantoras Sertanejas - YouTube", 
         trimestre = as.Date(format(quarterYear,"%m"))) %>%
  group_by(Nome, quarterYear, trimestre) %>%
...
#Geral
streamgraph(resume2, "Nome", "n", "quarterYear", offset="zero")%>%
  sg_axis_x(tick_interval=4, tick_units="month") %>% 
  sg_axis_y(1, 10)


#Dividido por cantoras
resume1 <- resume1 %>%
  group_by(ano, quarterYear) %>%
  summarise(total_views = as.integer(mean(n)))

#consumo geral dividor por media trimestral
streamgraph(resume1, "quarterYear", "total_views", "ano", offset = 'zero') %>% 
  sg_axis_x(tick_interval=4, tick_units="%Y") %>% 
  sg_axis_y(1,1) %>% 
  sg_legend(TRUE, "Intervalo: ")
